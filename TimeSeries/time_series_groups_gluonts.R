...
ga_page_raw_tbl %>%
  group_by(pagePath) %>%
  plot_time_series(date, pageViews, .facet_ncol = 4)
...
full_data_tbl <- ga_page_raw_tbl %>%
  ...
  tk_augment_fourier(
    .date_var = date,
    .periods = c(.5 * FORECAST_HORIZON, FORECAST_HORIZON),
    .K = 1
  ) %>%
...
data_prepared_tbl <- full_data_tbl %>%
  filter(!is.na(pageViews)) %>% drop_na()
...
recipe_spec_gluon <-  recipe(pageViews ~ pagePath + date + rowid,
                             data = training(splits)) %>%
                    update_role(rowid, new_role = "indicator")

recipe_spec_gluon %>% prep() %>%  juice()

#check the roles of each feature
recipe_spec_gluon %>% summary()
...
model_tbl_submodels %>% modeltime_accuracy(testing(splits))

# Forecast Visualization
forecast_submodel_test_tbl <- model_tbl_submodels %>% 
  modeltime_forecast(
    new_data = testing(splits),
    actual_data = data_prepared_tbl,
    keep_data = TRUE
 ) 
forecast_submodel_test_tbl %>%
  group_by(pagePath) %>%
...
forecast_submodel_test_tbl %>%  skim()
